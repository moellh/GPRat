...
#SBATCH --job-name="gp_X4"
#SBATCH --output=job_gprat-X4.out
...
# Test X4
echo "=== Starting Test X4 (GPRat) ==="
cd $gprat_dir
./compile_gpxpy_python_simcl1.sh -DGPXPY_WITH_CUDA=ON -DGPRAT_PREDICT_FULL_COV_STEPS=ON
cd experiment/test-X4-predict-fullcov-gprat/
mkdir -p apex-cpu
mkdir -p apex-gpu
./run_simcl1.sh
timestamp=$(date +"%m-%d_%H-%M-%S")
results_dir=$HOME/results/X4/${timestamp}
mkdir -p ${results_dir}
