...
#SBATCH --job-name="gp_X3"
#SBATCH --output=job_gprat-X3.out
...
# Test X3
echo "=== Starting Test X3 (GPRat) ==="
cd $gprat_dir
./compile_gpxpy_python_simcl1.sh -DGPXPY_WITH_CUDA=ON
cd experiment/test-X3-predict-fullcov-gprat/
mkdir -p apex-cpu
mkdir -p apex-gpu
./run_simcl1.sh
timestamp=$(date +"%m-%d_%H-%M-%S")
results_dir=$HOME/results/X3/${timestamp}
mkdir -p ${results_dir}
