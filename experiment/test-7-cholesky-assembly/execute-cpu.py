...
def execute(n_cores, n_train, n_tiles, n_reg, n_loops):
    # setup logging
    ...
    for i_loop in range(n_loops):
        single_run(output_writer, n_cores, n_train, n_tiles, n_reg, i_loop)
    gpx.stop_hpx()
    logger.info(f"completed run: {n_cores}, {n_train}, {n_tiles}, {n_reg}, {n_loops}")
    if os.path.isfile("apex_profiles.csv"):
        new_filename = f"apex-cpu/apex_profiles_{n_cores}_{n_train}_{n_tiles}_{n_reg}_{n_loops}.csv"
