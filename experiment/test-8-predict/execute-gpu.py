...
def execute(n_cores, n_train, n_test, n_tiles, n_reg, n_streams, n_loops):
    ...
    if os.path.isfile("apex_profiles.csv"):
        new_filename = (
            f"apex-gpu/apex_profiles_{n_cores}_{n_train}_{n_test}_{n_tiles}_{n_reg}_{n_streams}_{n_loops}.csv"
        )
...
def single_run(csv, n_cores, n_train, n_test, n_tiles, n_reg, n_streams, i_loop):
    ...
    gp_gpu = gpx.GP(
        train_in.data, train_out.data, n_tiles, n_tile_size, trainable=[True, True, True], n_streams=n_streams
    )

    pred_t = time.time()
    _ = gp_gpu.predict(test_in.data, m_tiles, m_tile_size)
